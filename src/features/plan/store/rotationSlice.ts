...
const current = (state: RotationSliceState, programId: string) => state.byProgram[programId] ?? createRotationState(programId);
...
const rotationSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(appReset, () => initialState).addCase(backupRestored, (_, action) => action.payload.rotation);
  },
});

export const { rotationSessionCompleted, rotationJumped } = rotationSlice.actions;
